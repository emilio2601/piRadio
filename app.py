...
class Recording(db.Model):
    ...
    def __init__(self, title, tunerID, date_start, duration, frequency, format):
        self.title = title
...
def validate_user_input(post_data):
    ...
    if not post_data.json: abort(400)
        ...
        if not "date_start" in post_data.json or not "title" in post_data.json or not "duration" in post_data.json or not "frequency" in post_data.json:
            abort(400)
        else:
            ...
            frequency = post_data.json["frequency"]
            format = post_data.json.get("format", "mp3")
            tunerID = post_data.json.get("tunerID", 1)
            return Recording(title, tunerID, date_start, duration, frequency, format)
...
def validate_user_input_update(post_data, existing):
    if not post_data: abort(400)
        ...
        frequency = post_data.json.get("frequency", existing.frequency)
        duration = post_data.json.get("duration", existing.duration)
        done = post_data.json.get("done", existing.done)
        rec = Recording(title, tunerID, date_start, duration, frequency, format)
        rec.id = existing.id
        rec.done = done
        return rec
